...
import { TimeEvents } from '../services/TimeService'; // Import TimeEvents
...
export default class RestingState implements NpcState {
    ...
    private handleDayStarted(): void {
        ...
        if (this.npc.workPosition) {
            ...
            const targetVec = new Phaser.Math.Vector2(this.npc.workPosition.x, this.npc.workPosition.y);
            // Transition to MovingState to go to work
            this.npc.changeState(new MovingState(), { targetPosition: targetVec, purpose: 'MovingToWork' });
        } else {
